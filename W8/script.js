...
function createMyChart() {
    ...
    // Collects restaurant types and converts to array without repeats
    let chartLabelsRaw = restaurants.map(function(restaurant){
        return restaurant.cuisine;
    })
    const chartLabelsSet = new Set(chartLabelsRaw);
    const chartLabels = Array.from(chartLabelsSet);

    const chartData = [];
    chartLabels.forEach(function(cuisine){
        chartData.push(cuisineCounts[cuisine]);
    })
...
function createMyMap() {
    ...
    try {
        ...
        restaurants.forEach(function(restaurant, index) {
            ...
            // const popupContent = ...;
            const popupContent = "<h4>" + restaurant.name + "</h4>" + restaurant.cuisine + "<br>" + restaurant.rating + "⭐";
